...
listed inside an ``Adress.txt`` file. Both FastHenry (``.inp``) and FastCap
...
from TXT_to_FastSolver.FastCapConverter import WireSections_to_FastCap_txt as fastcap
from TXT_to_FastSolver.FastHenryConverter import (
    WireSections_to_FastHenry_inp as fasthenry,
)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        nargs="?",
        type=Path,
        help="Path to Adress.txt (list of directories containing Wire_Sections.txt)",
    )
...
def main() -> None:
    ...
    if args.address_file is None:
        ...
        address_file = _prompt_path("Path to Adress.txt")
    else:
