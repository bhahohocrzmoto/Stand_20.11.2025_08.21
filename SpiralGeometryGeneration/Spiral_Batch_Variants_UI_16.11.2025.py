...
# NEW HELPER: write Adress.txt in the mother folder
def write_address_file(mother: Path, subfolders: List[Path], filename: str = "Adress.txt") -> None:
    """
    ...
    ----------
    mother : Path
        The "mother" folder selected in the GUI. Adress.txt is created here.

    subfolders : List[Path]
        List of subfolder paths created during the batch run
        (e.g. mother / "K1_N1.00", mother / "K1_N1.50", ...).

    filename : str, default "Adress.txt"
    ...
    def on_generate(self):
        ...
        for K in ks:
            for N in ns:
                ...
                # Remember this subfolder so we can later write it into Adress.txt
        ...
        # After generating all variants, write Adress.txt in the mother folder
